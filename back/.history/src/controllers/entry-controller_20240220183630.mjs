...
const getAvgHoursSleptByUserId = async (req, res) => {
  try {
    ...
    console.log(typeof userId); // this is string
    const userIdNumber = parseInt(userId, 10); // change to number for comparation of both, otherwise this function do not work
    console.log(typeof userIdNumber)
    console.log(typeof loggedInUserId); // this is number

    // Check if the logged-in user matches the requested user ID
    if (userIdNumber !== loggedInUserId) {
