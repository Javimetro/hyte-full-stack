...
const insertEntry = async (entry) => {
    try {
            ...
            const params = [entry.userId, entry.entryDate, entry.mood, entry.weight, entry.sleepHours, entry.notes];
...
const updateEntryById = async (entry) => {
    try {
        ...
        const params = [entry.mood, entry.weight, entry.sleepHours, entry.notes, entry.entry_id];
        const [result] = await promisePool.query(sql, params);
        console.log(result);
