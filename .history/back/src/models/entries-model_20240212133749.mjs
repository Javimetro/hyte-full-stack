...
const listAllEntries = async () => {
  try {
    const sql = 'SELECT user_id, username, user_level FROM Entries';
...
const insertEntrie = async (user) => {
  try {
    const sql = 'INSERT INTO Entries (username, password, email) VALUES (?, ?, ?)';
...
const updateUserById = async (user) => {
  try {
    const sql = 'UPDATE Entries SET username=?, password=?, email=? WHERE user_id=?';
...
const deleteUserById = async (id) => {
  try {
    const sql = 'DELETE FROM Entries WHERE user_id=?';
    const params = [id];
    const [result] = await promisePool.query(sql, params);
    console.log(result);
...
export {listAllEntries, selectUserById, insertUser, updateUserById, deleteUserById};
